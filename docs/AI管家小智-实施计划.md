...
#### 1.3 小智Claude Agent核心（30-45分钟）
- [ ] 实现小智入口 `src/index.ts`
- [ ] 实现小智核心 `src/core/xiaozhi.ts`
- [ ] 配置System Prompt
- [ ] **验证**: 飞书发消息，小智能用Claude回复
...
- [ ] **验证**: 完整流程：飞书→小智→Worker→通知

#### 3.2 Systemd部署（20分钟）
- [ ] 编写 `xiaozhi.service`
- [ ] 编写安装脚本 `install-xiaozhi.sh`
- [ ] 创建环境变量模板
- [ ] **验证**: systemctl start xiaozhi 正常启动
...
```
□ 1. mkdir -p xiaozhi/src/{core,feishu,session,worker,storage,utils}
□ 2. mkdir -p xiaozhi/{scripts,config}
□ 3. 初始化 package.json 和 tsconfig.json
□ 4. 安装依赖
□ 5. 实现 src/feishu/client.ts
□ 6. 实现 src/feishu/types.ts
□ 7. 实现 src/core/xiaozhi.ts
□ 8. 实现 src/index.ts
□ 9. 测试飞书消息收发
...
```bash
# Step 1: 创建目录
mkdir -p xiaozhi/src/{core,feishu,session,worker,storage,utils}
mkdir -p xiaozhi/{scripts,config}
