    ...
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/var/lib/xiaozhi/scripts/notify_xiaozhi.sh ${XIAOZHI_WORKER_ID}"
          }
        ]
      }
    ],
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/var/lib/xiaozhi/scripts/worker_completed.sh ${XIAOZHI_WORKER_ID}"
          }
        ]
      }
    ],
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/var/lib/xiaozhi/scripts/subagent_notify.sh ${XIAOZHI_WORKER_ID}"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_name' >> /var/lib/xiaozhi/workers/${XIAOZHI_WORKER_ID}/actions.log"
          }
        ]
      }
    ]
  ...
  async spawnWorker(config: WorkerSpawnConfig): Promise<ClaudeWorker> {
    ...
    const worker: ClaudeWorker = {
      ...
      hooks: {
        ...
        notifyScript: path.join(this.scriptsDir, 'notify_xiaozhi.sh'),
  ...
  private generateHooksConfig(workerId: string): HooksConfig {
    return {
      Notification: [{
        ...
        hooks: [{
          type: 'command',
          command: `${this.scriptsDir}/notify_xiaozhi.sh ${workerId}`,
        }],
      }],
      Stop: [{
        matcher: '',
...
### 3.1 notify_xiaozhi.sh

```bash
#!/bin/bash
# /var/lib/xiaozhi/scripts/notify_xiaozhi.sh
# Worker向小智发送进度通知

WORKER_ID="$1"
XIAOZHI_HOST="${XIAOZHI_HOST:-localhost}"
XIAOZHI_PORT="${XIAOZHI_PORT:-8080}"
...
# 同时写入进度文件（备用）
PROGRESS_FILE="/var/lib/xiaozhi/workers/${WORKER_ID}/progress.json"
...
```bash
#!/bin/bash
# /var/lib/xiaozhi/scripts/worker_completed.sh
# Worker完成时通知小智
...
# 保存完整结果
RESULT_FILE="/var/lib/xiaozhi/workers/${WORKER_ID}/result.json"
...
```bash
#!/bin/bash
# /var/lib/xiaozhi/scripts/subagent_notify.sh
# 子代理完成时通知
...
```yaml
# /var/lib/xiaozhi/config/config.yaml
server:
  host: "0.0.0.0"
  port: 8080
  hooks_port: 8081
...
worker:
  base_dir: "/var/lib/xiaozhi"
  scripts_dir: "/var/lib/xiaozhi/scripts"
...
storage:
  type: "sqlite"
  sqlite:
    path: "/var/lib/xiaozhi/data/xiaozhi.db"
  # redis:
  #   host: "localhost"
  #   port: 6379
  #   db: 0
...
logging:
  level: "info"
  format: "json"
  output: "/var/log/xiaozhi/xiaozhi.log"
```

### 4.2 AI小智的任务处理能力

AI小智本质上是一个**Claude Agent实例**，具备Claude的全部能力。它会根据任务复杂度自动选择处理方式：
...
if ! id "$XIAOZHI_USER" &>/dev/null; then
    useradd -r -s /bin/bash "$XIAOZHI_USER"
...
mkdir -p "$XIAOZHI_LOG"
chown -R "$XIAOZHI_USER:$XIAOZHI_USER" "$XIAOZHI_HOME" "$XIAOZHI_LOG"
echo "✓ 创建目录结构"

# 3. 复制脚本
cp scripts/*.sh "$XIAOZHI_HOME/scripts/"
...
echo ""
echo "=== 安装完成 ==="
echo "1. 编辑 /etc/xiaozhi/environment 填写配置"
echo "2. 运行 sudo systemctl start xiaozhi 启动服务"
echo "3. 运行 sudo journalctl -u xiaozhi -f 查看日志"
```
...
async function main() {
  ...
  const xiaozhi = new XiaoZhiService({
    ...
    storage: {
      type: 'sqlite',
      path: process.env.XIAOZHI_DB || '/var/lib/xiaozhi/data/xiaozhi.db',
    },
    feishu: {
    ...
    worker: {
      baseDir: process.env.XIAOZHI_HOME || '/var/lib/xiaozhi',
      scriptsDir: '/var/lib/xiaozhi/scripts',
    },
  });

  // 2. 启动Hooks接收器（接收Worker通知）
  const hooksReceiver = new HooksReceiver(
    xiaozhi.workerManager,
    xiaozhi.feishuClient,
    xiaozhi.sessionManager
  );
