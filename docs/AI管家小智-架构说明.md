...
**小智 (Xiaozhi)** 是一个基于 Claude CLI 的智能管家系统，通过飞书提供对话接口，拥有 AI 专家团队处理特定类型任务。
...
```
小智 → POST /expert/call {expert, task, workDir}
...
| 配置项 | 环境变量 | 默认值 |
|--------|---------|--------|
| 模型 | XIAOZHI_MODEL | glm-5 |
| 小智目录 | XIAOZHI_HOME | ~/.xiaozhi |
| HTTP 端口 | XIAOZHI_PORT | 8081 |
| API Token | XIAOZHI_API_TOKEN | null |
