...
### 1.1 项目目标
构建一个与飞书通信的 **AI管家艾克斯**，基于 Claude Agent SDK 实现，作为用户的智能助手：
- 接收飞书消息，与用户沟通
- 处理一般性事务（简单任务直接处理）
...
| F2 | 艾克斯直接处理简单任务 | P0 | 作为Claude Agent处理问答、文件读取、简单编辑等 |
| F3 | 创建Worker处理复杂任务 | P0 | 在tmux中启动claude CLI处理长期任务 |
| F4 | Claude Hooks进度通知 | P0 | Worker通过Hooks主动通知艾克斯进度 |
...
### 2.1 系统角色
```
用户 ←→ 飞书机器人 ←→ AI管家艾克斯(Claude Agent) ←→ Claude AI Workers
                              ↓                           ↓
...
const xdevAgent = new ClaudeAgent({
  model: 'claude-sonnet-4-5-20250929',
  systemPrompt: `你是AI管家艾克斯，负责：
1. 与用户通过飞书沟通
...
**设计**: Worker通过Hooks主动通知艾克斯，避免轮询

```json
// Worker的Hooks配置
{
  "hooks": {
    "Notification": [{ "matcher": "", "hooks": [{ "type": "command", "command": "notify_xdev.sh" }] }],
...
```bash
sudo systemctl start xdev    # 启动
sudo systemctl stop xdev     # 停止
sudo systemctl status xdev   # 状态
sudo journalctl -u xdev -f   # 日志
```
