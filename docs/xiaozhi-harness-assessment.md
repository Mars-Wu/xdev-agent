...
> 基于 OpenAI 和 Anthropic 的 Harness 工程最佳实践，对小智项目框架结构进行审视
...
**总体评价**：小智项目已经具备了一些 Harness 工程的基础能力（专家系统、会话管理、SQLite 持久化），但在长时运行 Agent 的状态管理和自验证方面有明显短板。
...
```typescript
// 在每个 workDir 创建 .xiaozhi-progress.md
# 工作进度日志
...
| 更新 System Prompt | `~/.xiaozhi/system-prompt.md` | 1h |
...
```typescript
// 专家完成时生成
// workDir/.xiaozhi-session-{timestamp}.md
# 会话总结
...
小智项目已经具备了 Harness 工程的**基础设施**（专家系统、持久化、会话管理），但在**状态可读性**和**自验证**方面需要加强。
