...
从当前代码和配置看，艾克斯是一个**飞书应用机器人**：

- `xdev/src/feishu/client.ts` 使用 `@larksuiteoapi/node-sdk`
- 通过 `WSClient` 订阅 `im.message.receive_v1`
- 收到飞书消息后进入 `xdev/src/index.ts` 的 `handleMessage(...)`
- 回复通过 `client.im.message.create(...)` 发回飞书

仓库文档 `xdev/docs/GUIDE.md` 也明确写了：

1. 在飞书中找到艾克斯机器人
2. 直接发送消息

也就是说，**艾克斯的主交互入口本来就是飞书 IM。**

## 8. 当前环境评估：已验证可以通过飞书 CLI 给艾克斯发消息

### 8.1 已确认的事实

1. **艾克斯服务当前在运行中**  
   `systemctl --user status xdev` 显示服务为 `active (running)`。
...
4. **当前 lark-cli 配置的应用，与艾克斯运行所用应用不是同一个**  
   本机 `lark-cli` 当前解析到的是一个独立的 app；`xdev/.env` 中配置的是另一个 app。

5. **按会话名不一定能直接搜到“艾克斯”**  
   即使切到 `--as user`，`lark-cli im +chat-search --query 艾克斯` 也可能返回 0 条；机器人 P2P 会话在搜索和列举上的表现与普通群聊并不完全一致。

6. **已完成两次实际发送验证**  
   一次是以 `--as user` 向“飞书新用户体验群”发送测试消息，并在手机客户端确认同步可见；另一次是先向艾克斯发送一条消息，再通过消息搜索反查出与艾克斯的 P2P `chat_id`，随后确认可以直接使用该 `chat_id` 发送消息。

### 8.2 结论

**当前环境已经验证：可以通过飞书 CLI 给艾克斯发消息。**
...
# 如果能直接搜到会话
lark-cli im +chat-search --as user --query "艾克斯" --format table
lark-cli im +messages-send --as user --chat-id <xdev_chat_id> --text "你好，艾克斯"
```
...
# 再从消息记录反查会话
lark-cli im +messages-search --as user --query "你好，艾克斯" --format json

# 得到 chat_id 后直接发送
lark-cli im +messages-send --as user --chat-id <xdev_chat_id> --text "继续测试"
```

这条路径已经在当前环境中验证成功。

#### 路径 B：让 lark-cli 和艾克斯共用同一个应用

适用场景：希望 CLI 直接站在“艾克斯这个 bot”的身份上工作。

做法是把 `lark-cli config init` 指向与艾克斯服务相同的飞书应用，再补齐需要的 IM scopes。

这条路径更像“让 CLI 直接操控艾克斯的 bot 身份”，适合联调和自动化，但要注意：
...
- **当前这台机器已经可以通过飞书 CLI 与艾克斯对话**

当前最稳妥的使用方式，是优先走**路径 A（用户身份）**。如果后续再次遇到“按名称搜不到艾克斯会话”，不要先怀疑消息链路，优先用 `+messages-search` 从已有消息反查 `chat_id`。

## 9. 当前仓库里的 lark-cli 工具层集成状态

截至当前版本，`xdev/` 已经把 `lark-cli` 接成一组可供 Agent 直接调用的结构化工具，定位是**飞书能力工具层**，而不是消息入口。
...
- **飞书直连 SDK / WebSocket**：负责艾克斯收消息、回消息
- **lark-cli 工具层**：负责让艾克斯主动调用飞书能力
...
也就是说，**代码层面日历工具已接入，艾克斯已经会调用；当前阻塞点是本机 `lark-cli` 用户授权范围，而不是代码实现。**
