...
**存储路径**：`~/.xdev/topics/{topicId}/history.json`
...
```
已知话题（最近活跃优先）：
[T1] 类型:code_task  摘要:"开发艾克斯AI助手，TypeScript+飞书"
     实体:["xdev","feishu"]  最近活跃:30分钟前  轮次:156
...
describe('MessageRouter - routeAndAssemble', () => {
  ...
  beforeEach(() => {
    ...
    mockTopicGraph.getActiveSummaries.mockResolvedValue([
      {
        id: 'T1', type: 'code_task', summary: '开发艾克斯AI助手，TypeScript+飞书',
        entityTags: ['xdev', 'feishu'], updatedAt: Date.now() - 30 * 60 * 1000
      },
      {
  ...
  test('正确路由到已有话题 T3（核心场景：TqQuant）', async () => {
    ...
    // 确保未注入全局 history（xdev 相关内容不应出现）
    const allContent = JSON.stringify(result.messages);
    expect(allContent).not.toContain('xdev');
  });
  ...
  test('historyStrategy=full 时加载完整 history', async () => {
    ...
    mockLLMClient.chatSync.mockResolvedValue({
      content: JSON.stringify({
        ...
        entityTags: ['xdev'], confidence: 0.99
      }),
      toolCalls: []
    });
...
describe('Phase 4 流水线集成测试', () => {
  ...
  test('【核心场景】TqQuant 不被 xdev history 污染', async () => {
    // 预设：T1 有大量 xdev 相关 history
    const t1History = new MessageHistoryManager({ maxMessages: 200 });
    for (let i = 0; i < 50; i++) {
      t1History.addMessage({ role: 'user', content: `艾克斯开发任务${i}` });
    ...
    await topicGraph.updateSummary('T1', '开发艾克斯AI助手，TypeScript+飞书', ['xdev', 'feishu']);
...
describe.skip('Phase 4 E2E（需要真实 API Key）', () => {
  test('TqQuant 查询 → 正确路由到 T3（真实 glm-4.7-flash）', async () => {
    ...
    expect(result.topicId).not.toBe('T1'); // 不能路由到 xdev 话题
...
# 话题 history 目录
XDEV_TOPICS_DIR=~/.xdev/topics
```

`handleMessage` 中：
```typescript
if (process.env.XDEV_TOPIC_ROUTING === 'true') {
  // 3 阶段流水线
} else {
...
### 定量指标
- [ ] TqQuant 查询测试（3.6 集成测试）：100% pass，xdev history 不污染
- [ ] 序列化往返测试（3.1）：数据 0 丢失
