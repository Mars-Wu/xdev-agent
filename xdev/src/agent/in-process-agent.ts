...
export class InProcessAgent {
  ...
  private getBasePrompt(): string {
    const typePrompts: Record<AgentType, string> = {
      'general-purpose': `你是艾克斯的协作 Agent ${this.config.name}。

你是一个通用助手，可以执行各种任务。
简洁友好地完成任务，不要过多客套。`,

      'explore': `你是艾克斯的研究 Agent ${this.config.name}。
...
你的输出应该结构清晰、结论明确。`,
      ...
      'plan': `你是艾克斯的规划 Agent ${this.config.name}。
...
你的输出应该有条理、可执行。`,
      ...
      'coder': `你是艾克斯的编程专家 Agent ${this.config.name}。
...
5. **说明理由**：每处关键改动附简短注释说明为何这样改。`,
  ...
  private async makeDecision(question: string): Promise<string> {
    ...
    const response = await this.llmClient.chatSync({
      ...
      system: `你是艾克斯的决策 Agent。请简洁地回答问题，给出你的决策和建议。`,
    });
