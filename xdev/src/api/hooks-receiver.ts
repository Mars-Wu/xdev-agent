...
export class HooksReceiver {
  ...
  constructor() {
    this.app = express();
    this.apiToken = process.env.XDEV_API_TOKEN || null;
    if (this.apiToken) {
  ...
  private requireAuthToken(req: Request, res: Response, next: NextFunction): void {
    ...
    const providedToken = req.headers['x-xdev-token'] as string;
  ...
  private requireStrictAuthToken(req: Request, res: Response, next: NextFunction): void {
    if (!this.apiToken) {
      res.status(503).json({ error: 'XDEV_API_TOKEN 未配置，测试入口已禁用' });
      return;
    }
   ...
   * 注入消息到艾克斯完整处理管道（话题路由→记忆注入→agent loop→发送飞书）
