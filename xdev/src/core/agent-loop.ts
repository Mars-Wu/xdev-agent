...
import { getWorkflowRuntime } from '../tools/workflow-runtime';
...
export const DEFAULT_MAX_TURNS = parseInt(process.env.XDEV_MAX_TURNS || '30');
...
async function injectWorkflowResumeReminder(
  historyManager: MessageHistoryManager,
  workDir: string,
): Promise<void> {
  const recentText = historyManager.getMessages()
    .slice(-5)
    .map(msg => typeof msg.content === 'string' ? msg.content : '')
    .join('\n');
  if (recentText.includes('<workflow-reminder>')) {
    return;
  }
  ...
  try {
    ...
    const workflows = runtime.getResumableWorkflows(workDir);
    ...
    const summary = workflows
      ...
      .map(workflow => {
        const stage = workflow.currentStageId
          ...
          : undefined;
        return `- ${workflow.title} (${workflow.id})：${workflow.status} / ${stage ? stage.name : '已完成'}`;
      })
      .join('\n');
    ...
    historyManager.addMessage({
      role: 'user',
      content:
        `<workflow-reminder>检测到当前目录下存在 ${workflows.length} 个可恢复工作流：\n` +
        `${summary}\n` +
        `如需继续，可使用 workflow 工具查看详情、checkpoint 或 resume。</workflow-reminder>`,
    });
    logger.info(`[Agent Loop] 注入 ${workflows.length} 个工作流恢复提醒`);
  } catch (err) {
    logger.warn(`[Agent Loop] 工作流恢复提醒注入失败: ${err}`);
  }
}
...
function isLowSignalToolAcknowledgement(text: string): boolean {
  const compact = text.trim()
  if (!compact) return true
  return /^(已记住|记住了|好的，?已记录|话题摘要已更新|记忆已保存|已保存|任务完成|已完成|内部记录已完成|后续会按这个框架)/.test(compact)
}
...
export async function runAgentLoop(
  ...
  await injectWorkflowResumeReminder(historyManager, process.cwd());
  ...
  while (turns < maxTurns) {
    ...
    if (response.content) {
      ...
      if (response.content.trim().length > 20 && !isLowSignalToolAcknowledgement(response.content)) {
