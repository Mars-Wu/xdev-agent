...
输出 JSON，格式：
{
  "newEntityTags": [
    "实体1",
    { "name": "实体2", "confidence": 0.72, "reason": "在摘要中高频出现" }
  ],
  "topicRelations": [
    {
      "toTopicId": "T1",
  ...
  try {
    ...
    return {
      ...
      episodicPattern: {
        ...
        confidence: parsed.episodicPattern?.confidence,
        reason: parsed.episodicPattern?.reason,
        machineVerified: parsed.episodicPattern?.machineVerified,
      },
    }
  } catch {
    return emptyResult()
...
async function applyBgResult(
  ...
  if (result.newEntityTags.length > 0) {
    topicGraph.updateEntityTags(
      topicId,
      result.newEntityTags.map(tag => typeof tag === 'string' ? tag : tag.name).filter(Boolean),
    )
  }
  ...
  for (const rel of result.topicRelations) {
    if (rel.weight > 0.2) {
      topicGraph.upsertRelation(topicId, rel.toTopicId, rel.relation, rel.weight, {
        ...
        provenance: rel.provenance || 'llm_inferred',
  ...
  if (result.episodicPattern.shouldSave && result.episodicPattern.approach) {
    ...
    await memoryManager.addMemory({
      ...
      metadata: {
        ...
        confidence: result.episodicPattern.confidence,
        provenance: 'background_pass',
        extractionReason: result.episodicPattern.reason,
        machineVerified: Boolean(result.episodicPattern.machineVerified),
      },
    })
  }
}
...
async function runMemoryExtraction(
  ...
  let items: Array<{ content: string; category: string; importance: number; confidence?: number; reason?: string }>
  try {
    ...
    try {
      await memoryManager.addMemory({
        ...
        metadata: {
          ...
          extractionReason: item.reason || '从多轮对话中抽取',
          machineVerified: false,
          sourceMessageRange: {
            startIndex: 0,
            endIndex: Math.max(0, lines.length - 1),
          },
          sourceSampleCount: lines.length,
        },
      })
      saved++
    } catch {
