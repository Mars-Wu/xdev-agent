...
export interface RecentTurnSignal {
  topicId: string
  userMessage: string
...
如果提供了“最近连续对话信号”，可用它辅助理解“刚才/继续/这个/那个/上一个”等模糊回指，但不要机械继承；只有在最近对话与当前消息明显连续时才承接原话题。
...
const CONTINUITY_SYSTEM_PROMPT = `你是连续会话判断器。你的任务是根据最近 1~2 轮对话信号，判断当前消息是否在承接最近的话题。

只在以下情况返回 shouldContinue=true：
- 当前消息包含“刚才/继续/这个/那个/它/上一个/前面/刚刚”等模糊回指，或明显依赖最近对话才能理解
- 最近一轮对话足以补全当前消息含义
- 延续最近话题比切到旧话题或新话题更合理
...
const CONTINUITY_WINDOW_MS = 15 * 60 * 1000
...
export async function routeAndAssemble(
  ...
  try {
    ...
    // 2. 若消息像是在承接上一轮，则先用最近 1~2 轮做连续性判断
    const recentContext = normalizeContinuityContext(continuity)
    const heuristicContinuation = buildHeuristicContinuation(userMessage, recentContext)
    if (heuristicContinuation) {
      return Promise.all([
        buildSubContext(heuristicContinuation, activeSummaries, topicGraph, memoryManager),
      ])
    }

    if (shouldEvaluateContinuity(userMessage, recentContext)) {
      const continuation = await evaluateContinuity(userMessage, recentContext!, llmClient)
      if (continuation) {
        const contexts = await Promise.all([
          buildSubContext(continuation, activeSummaries, topicGraph, memoryManager),
        ])
        return contexts
      }
    }
    ...
    // 3. LLM 路由分类（返回多路由结果）
    const heuristicRoutes = buildExplicitSplitRoutes(userMessage)
    if (heuristicRoutes.length > 1) {
      logger.info(`多话题拆分: ${heuristicRoutes.length} 个子任务 (显式编号问题启发式)`)
      return Promise.all(
        heuristicRoutes.map(route => buildSubContext(route, activeSummaries, topicGraph, memoryManager))
      )
    }

    const multiRoute = await classifyMessage(userMessage, activeSummaries, llmClient, recentContext)
...
async function classifyMessage(
  ...
  const continuityText = formatContinuityForRouter(continuity)
  const userPrompt = continuityText
    ? `最近连续对话信号（仅供判断是否承接上一轮，不等于完整历史）：\n${continuityText}\n\n已知话题（按最近活跃排序）：\n${summaryText}\n\n用户消息：${userMessage}`
    : `已知话题（按最近活跃排序）：\n${summaryText}\n\n用户消息：${userMessage}`
...
function normalizeContinuityContext(continuity?: ContinuityContext): ContinuityContext | undefined {
  if (!continuity?.recentTurns?.length) return undefined
  const now = Date.now()
  const recentTurns = continuity.recentTurns
    .filter(turn => now - turn.timestamp <= CONTINUITY_WINDOW_MS)
...
function shouldEvaluateContinuity(
  userMessage: string,
  continuity?: ContinuityContext,
): boolean {
  if (!continuity?.recentTurns?.length) return false
  const compact = userMessage.trim()
  if (compact.length <= 24) return true
  return hasStrongFollowUpHint(compact)
}

function buildHeuristicContinuation(
  userMessage: string,
  continuity?: ContinuityContext,
): RouteResult | null {
  if (!continuity?.recentTurns?.length) return null
  const latestTurn = continuity.recentTurns[0]
  const rewrittenMessage = rewriteFollowUpWithRecentReply(userMessage, latestTurn.assistantReply)
  if (!rewrittenMessage) return null

  return {
    topicId: latestTurn.topicId,
    isNewTopic: false,
    subMessage: rewrittenMessage,
    historyStrategy: 'full',
    historyHint: '显式续问上一轮回答，使用启发式续接',
    relatedTopicIds: [],
    entityTags: [],
    confidence: 0.96,
  }
...
function buildExplicitSplitRoutes(userMessage: string): RouteResult[] {
  const subQuestions = splitExplicitSubQuestions(userMessage)
  if (subQuestions.length <= 1) return []

  return subQuestions.map(subMessage => {
    const entityTags = Array.from(
      new Set(
        ['xdev', 'workflow', 'doctor']
          .filter(tag => subMessage.toLowerCase().includes(tag))
      )
    )
    const isCodeTask = /(修改|重构|修复|实现|编写|代码|文件|日志级别)/.test(subMessage)

    return {
      topicId: isCodeTask ? 'new:code_task' : 'new:project_query',
      isNewTopic: true,
      subMessage,
      historyStrategy: 'none',
      historyHint: '显式编号问题，按独立子问题拆分',
      relatedTopicIds: [],
      entityTags,
      confidence: 0.9,
    }
...
async function evaluateContinuity(
  ...
  const recentTurnsText = continuity.recentTurns
    .map((turn, index) => {
      const age = formatAge(Date.now() - turn.timestamp)
      return `最近第 ${index + 1} 轮 [topicId=${turn.topicId}, 最近活跃=${age}]
用户: ${turn.userMessage}
助手: ${turn.assistantReply}`
  ...
  const decision = parseContinuityDecision(response.content)
  if (!decision || !decision.shouldContinue || !decision.topicId || decision.confidence < 0.72) {
    return null
  }
  ...
  return {
    ...
    isNewTopic: false,
    subMessage: decision.rewrittenMessage || userMessage,
    historyStrategy: 'recent_20',
    historyHint: decision.reason || '承接最近连续会话',
    relatedTopicIds: [],
    entityTags: [],
...
function parseContinuityDecision(content: string): ContinuityDecision | null {
  const jsonMatch = content.match(/\{[\s\S]*\}/)
  ...
  try {
    ...
    return {
      ...
      rewrittenMessage: parsed.rewrittenMessage ? String(parsed.rewrittenMessage) : undefined,
...
function formatContinuityForRouter(continuity?: ContinuityContext): string {
  if (!continuity?.recentTurns?.length) return ''
  return continuity.recentTurns
    .map((turn, index) => {
      const age = formatAge(Date.now() - turn.timestamp)
      return `- 最近第 ${index + 1} 轮 [topicId=${turn.topicId}, 最近活跃=${age}]
  用户: ${turn.userMessage}
