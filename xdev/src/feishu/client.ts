...
export class FeishuClient {
  ...
  async sendCard(chatId: string, card: MessageCard): Promise<void> {
    try {
      await this.client.im.message.create({
        ...
        data: {
          receive_id: chatId,
          content: JSON.stringify(this.normalizeCardPayload(card)),
  ...
  private formatContent(reply: FeishuReply): string {
    if (reply.type === 'interactive' && reply.card) {
      return JSON.stringify(this.normalizeCardPayload(reply.card));
    }
    if (reply.type === 'post') {
  ...
  private normalizeCardPayload(card: MessageCard | (MessageCard & { card?: MessageCard; type?: string })): MessageCard {
    if (card && typeof card === 'object' && 'card' in card && card.card) {
      return card.card;
    }
    return card;
  }
