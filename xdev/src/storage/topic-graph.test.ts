...
import { TopicGraph, renderTopicGraphSnapshotMarkdown } from './topic-graph'
import { MessageHistoryManager } from '../core/message-history'

function makeTempGraph(): { tg: TopicGraph; tmpDir: string } {
  const tmpDir = path.join(os.tmpdir(), `xdev-tg-test-${Date.now()}-${Math.random().toString(36).slice(2)}`)
...
describe('TopicGraph', () => {
  ...
  describe('upsertRelation', () => {
    ...
    it('持久化 confidence / provenance / evidence', () => {
      tg.getOrCreate('T_meta_A', 'other')
      tg.getOrCreate('T_meta_B', 'other')
      tg.upsertRelation('T_meta_A', 'T_meta_B', 'related', 0.7, {
        confidence: 0.61,
        provenance: 'llm_inferred',
        evidence: 'shared module',
      })

      const [relation] = tg.getRelations('T_meta_A')
      expect(relation.confidence).toBe(0.61)
      expect(relation.provenance).toBe('llm_inferred')
      expect(relation.evidence).toBe('shared module')
    })
  })

  describe('snapshot export', () => {
    it('builds a topic snapshot and markdown report', () => {
      tg.getOrCreate('T_report_A', 'code_task')
      tg.getOrCreate('T_report_B', 'project_query')
      tg.updateSummary('T_report_A', 'Reviewing task graph', ['task', 'graph'])
      tg.upsertRelation('T_report_A', 'T_report_B', 'related', 0.4, {
        confidence: 0.8,
        provenance: 'llm_inferred',
      })

      const snapshot = tg.buildSnapshot()
      const markdown = renderTopicGraphSnapshotMarkdown(snapshot)

      expect(snapshot.summary.topicCount).toBe(2)
      expect(snapshot.summary.relationCount).toBe(1)
      expect(markdown).toContain('# Topic Graph Report')
      expect(markdown).toContain('T_report_A')
    })
  })
})
