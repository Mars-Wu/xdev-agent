...
export interface TaskGraphAnalysis {
  ...
  criticalPath: string[]
  blockedReasons: Array<{ taskId: string; waitingOn: string[] }>
}
...
function getDefaultConfig(): TaskSystemConfig {
  return {
    graphPath: path.join(PATHS.XDEV_HOME, 'tasks', 'task-graph.json'),
...
export class TaskSystem {
  ...
  buildGraphAnalysis(): TaskGraphAnalysis {
    const tasks = this.getAllTasks()
    const taskMap = new Map(tasks.map(task => [task.id, task]))
    const depthMemo = new Map<string, { depth: number; chain: string[] }>()
    const edges = tasks.flatMap(task => task.dependencies.map(dep => ({ from: dep, to: task.id })))

    const computeDepth = (taskId: string): { depth: number; chain: string[] } => {
      const cached = depthMemo.get(taskId)
      if (cached) return cached

      const task = taskMap.get(taskId)
      if (!task || task.dependencies.length === 0) {
        const base = { depth: 1, chain: task ? [task.id] : [] }
        depthMemo.set(taskId, base)
        return base
      }

      let best: { depth: number; chain: string[] } = { depth: 1, chain: [task.id] }
      for (const depId of task.dependencies) {
        const depDepth = computeDepth(depId)
        if (depDepth.depth + 1 > best.depth) {
          best = { depth: depDepth.depth + 1, chain: [...depDepth.chain, task.id] }
        }
      }
      depthMemo.set(taskId, best)
      return best
    }
    ...
    for (const task of tasks) {
      const candidate = computeDepth(task.id)
      if (candidate.depth > criticalPathLength) {
        criticalPathLength = candidate.depth
        criticalPath = candidate.chain
      }
    }
    ...
    return {
      ...
      blockedReasons: tasks
        ...
        .map(task => ({
          taskId: task.id,
          waitingOn: task.dependencies.filter(depId => taskMap.get(depId)?.status !== 'completed'),
        })),
    }
  }
...
export function renderTaskGraphAnalysisMarkdown(analysis: TaskGraphAnalysis): string {
  const lines = [
    '# Task Graph Report',
    '',
    `- 生成时间：${analysis.generatedAt}`,
    `- 总任务数：${analysis.summary.total}`,
    `- Ready：${analysis.summary.ready}`,
    `- Blocked：${analysis.summary.blocked}`,
    `- In Progress：${analysis.summary.inProgress}`,
  ...
  if (analysis.blockedReasons.length === 0) {
    ...
    for (const item of analysis.blockedReasons) {
      lines.push(`- ${item.taskId}: waiting on ${item.waitingOn.join(', ') || 'unknown'}`)
    }
  }

  lines.push('', '## Top Tasks', '')
  for (const task of analysis.tasks.slice(0, 10)) {
...
export function getTaskGraphArtifactPaths(): { jsonPath: string; markdownPath: string } {
  const baseDir = path.join(PATHS.CACHE_DIR, 'observability')
  return {
    jsonPath: path.join(baseDir, 'task-graph.json'),
    markdownPath: path.join(baseDir, 'task-report.md'),
  }
}
