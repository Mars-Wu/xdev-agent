...
// T9: Clarify 工具 — 等待飞书用户回复的 pending Map（chatId → resolve函数）
const pendingClarifyReplies = new Map<string, (text: string) => void>();
// AsyncLocalStorage 安全传递当前消息的 chatId（并发安全，无全局变量竞争）
const chatIdStorage = new AsyncLocalStorage<string>();

/**
 * 等待飞书指定 chatId 的用户下一条消息
 */
function waitForFeishuReply(chatId: string, timeoutMs: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      pendingClarifyReplies.delete(chatId);
      reject(new Error('等待用户回复超时（60秒）'));
    }, timeoutMs);

    pendingClarifyReplies.set(chatId, (text: string) => {
      clearTimeout(timer);
      pendingClarifyReplies.delete(chatId);
      resolve(text);
    });
...
async function main() {
  ...
  feishuClient.setMessageHandler(async (msg) => {
    ...
    const pendingResolver = pendingClarifyReplies.get(msg.chatId);
    if (pendingResolver) {
      pendingResolver(msg.content.trim());
      return;
    }
  ...
  setClarifyCallback(async (question: string, choices: string[] | null) => {
    ...
    return waitForFeishuReply(chatId, 60_000);
  });
  ...
  feishuClient.setCardActionHandler(async (chatId, actionValue) => {
    ...
    const resolver = pendingClarifyReplies.get(chatId);
    if (resolver) resolver(reply);  // resolver 内部已处理 delete + resolve
  });
...
async function handleMessage(
  ...
  try {
    ...
    if (msg.msgType !== 'image' && content.length > MAX_MESSAGE_LENGTH) {
    ...
    try {
      ...
      try {
        ...
        const fallbackText = imageDescription
          ? `[主人@飞书 发送了一张图片]\n\n**图片内容分析**:\n${imageDescription}${msg.content && msg.content !== '[图片]' ? `\n\n**用户问题**: ${msg.content}` : ''}`
          : `[主人@飞书] ${msg.content}`;
