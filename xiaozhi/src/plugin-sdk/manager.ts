...
export class PluginManager implements IPluginManager {
   ...
   * 注册插件（内置插件使用）
   */
  register(plugin: XiaozhiPlugin): boolean {
  ...
  async load(pluginPath: string): Promise<PluginLoadResult> {
    try {
      ...
      const plugin: XiaozhiPlugin = pluginModule.default || pluginModule;
   ...
   * 获取插件
   */
  get(pluginId: string): XiaozhiPlugin | undefined {
   ...
   * 创建插件上下文
   */
  private createContext(plugin: XiaozhiPlugin): PluginContext {
